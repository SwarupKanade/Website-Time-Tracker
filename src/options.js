...
function save_options() {
  ...
  var work_op = document.getElementById("work_op");
  var work_data = JSON.parse(localStorage["workdata"]);
  var work_status = work_data.status;
  var work_time = work_data.time;
  if (work_op.value != work_status) {
  	var dt = new Date();
  	var timeString = dt.getFullYear() +  "-" + dt.getMonth() + "-" + dt.getDate() + " " + dt.getHours() + ":" + dt.getMinutes() +":" + dt.getSeconds()
  	localStorage["workdata"] = JSON.stringify({
  		status : work_op.value,
  		time : timeString,
  	});
  	document.getElementById("work_status").innerHTML = "&emsp;&emsp;" + work_op.value.replace(/\b\w/g, l => l.toUpperCase()) + " working since " + timeString;
  }
...
function restore_options() {
  ...
  var work_data = JSON.parse(localStorage["workdata"]);
  var work_op = document.getElementById("work_op");
  work_op.value = work_data.status;
}
...
function clearData() {
  ...
  var work_data = localStorage["workdata"];
  localStorage.clear();
  localStorage["blacklist"] = blacklist;
  localStorage["workdata"] = work_data;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var work_data = JSON.parse(localStorage["workdata"]);
  document.getElementById("work_status").innerHTML = "&emsp;&emsp;" + work_data.status.replace(/\b\w/g, l => l.toUpperCase()) + " working since " + work_data.time;
});
